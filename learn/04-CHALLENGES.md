...
2. **Instrument the pipeline** with counters and histograms
   - `vigilo_requests_processed_total` (counter by stage)
   - `vigilo_threats_detected_total` (counter by severity)
   - `vigilo_inference_duration_seconds` (histogram)
   - `vigilo_queue_depth` (gauge per queue)
...
- `vigilo.raw` - Raw log lines (partitioned by hash of source IP)
- `vigilo.parsed` - Parsed log entries (same partition key)
- `vigilo.enriched` - Enriched feature vectors
- `vigilo.scored` - Final scored results for dispatch
